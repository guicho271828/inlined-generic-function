...
generic function is minimal. Although it currently supports only +=:around=
and primary methods of+ the standard method combination defined in ANSI CL,
empirical analysis showed that the resulting code is up to *10 times*
faster than the normal generic function invocation.

Edit (2016/12/22) : surprisingly, it actually *WORKS* with :before and :after method
