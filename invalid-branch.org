...
For example, using =invalid-branch= in a default method of inlined-generic-function, you can mimic the behavior of static type checking (only on sbcl). See example in [[t/invalid-branch-playground.lisp]].
